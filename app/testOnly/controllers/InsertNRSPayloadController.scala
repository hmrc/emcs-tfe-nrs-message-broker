...
class InsertNRSPayloadController @Inject()(cc: ControllerComponents,
                                           nrsSubmissionRecordsRepository: NRSSubmissionRecordsRepository,
                                          )(implicit ec: ExecutionContext) extends BackendController(cc) with Logging {

  def insertRecord(): Action[JsValue] = Action.async(parse.json) { implicit request =>
    withJsonBody[NRSPayload] {
      nrsPayload =>
        val submissionRecord = NRSSubmissionRecord(payload = nrsPayload)
        nrsSubmissionRecordsRepository.insertRecord(submissionRecord)
          .map(_ => Ok(""))
          .recover {
            case e =>
              logger.warn(s"[insertRecord] Failed to insert NRS payload with error: ${e.getMessage}")
